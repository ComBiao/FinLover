...
// The mocked auth logic hardcodes `mocked_user_id_from_token` — we must
// derive a valid 24-hex-char ObjectId from that string.
// ---------------------------------------------------------------------------
const MOCK_USER_ID = new mongoose.Types.ObjectId(
  Buffer.from('mocked_user_id_fr').toString('hex').slice(0, 24),
);
